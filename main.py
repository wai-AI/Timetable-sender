...
async def DownloadingPhotos(message: Message, id_group: int, day_of_week: str, week_type: str) -> str:
    if message.photo:
        file_id = message.photo[-1].file_id
        file_info = await message.bot.get_file(file_id)

        cursor.execute('''SELECT chat_name FROM KNEU WHERE id = ?''', (id_group,))
        res = cursor.fetchone()
        chat_name = res[0]

        dir_path = os.path.join(os.getcwd(), f"pic\{id_group}_{chat_name}")
        os.makedirs(dir_path, exist_ok=True)

        file_name = f"{day_of_week}{week_type}.png"
...
async def CancelStart(message: Message):
    try:
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 6</b>. Задля її усунення зверніться будь ласка до @Zakhiel")
...
async def CheckLinks(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 11</b>. Задля її усунення зверніться будь ласка до @Zakhiel")    
...
async def CheckEmails(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 13</b>. Задля її усунення зверніться будь ласка до @Zakhiel")    
...
async def GetTimetable(call: CallbackQuery) -> None:
    try: 
        ...
        await call.message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 14</b>. Задля її усунення зверніться будь ласка до @Zakhiel")    
...
async def ConfirmChangeAdmin(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 19</b>. Задля її усунення зверніться будь ласка до @Zakhiel")        
    ...
    try:
        ...
        if call.data.startswith("ConfirmChange_"):
            ...
            if id_new_admin and id_group:
                ...
                await call.message.answer("Сталася помилка під час зміни адміністратора. Перевірте ID користувача")
        else:
        ...
        if action == 'Lower':
            if type_user == 'Admin':
                ...
                if timetable == "Розкладу на понеділок нижнього тижня ще немає":
                    ...
                    await call.message.answer_photo(photo, "<b>Розклад на понеділок нижнього тижня</b>", show_caption_above_media=False, reply_markup=BackKb('WeekSelection', 'User', id_group))
        elif action == "Top":
            if type_user == 'Admin':
                await state.set_state(Form.MondayTimetable)
...
async def SetMonday(message: Message, state: FSMContext) -> None:
    try:
        ...
        week_type = "Нижній" if action == "Lower" else "Верхній"  

        path = await DownloadingPhotos(message, id_group, day_of_week, week_type)

        if path and id_group:
            await state.update_data(MondayTimetable=path)

            if action == "Lower":
                cursor.execute("""UPDATE KNEU SET timetable_monday_lower = ? WHERE id = ?""", (path, id_group))
                conn.commit()
                ...
                cursor.execute("""UPDATE KNEU SET timetable_monday_top = ? WHERE id = ?""", (path, id_group))
                conn.commit()
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 25</b>. Задля її усунення зверніться будь ласка до @Zakhiel")
        ...
        if action == 'Lower':
            if type_user == 'Admin':
                ...
                if timetable == "Розкладу на вівторок нижнього тижня ще немає":
                    ...
                    await call.message.answer_photo(photo, "<b>Розклад на вівторок нижнього тижня</b>", show_caption_above_media=False , reply_markup=BackKb('WeekSelection', 'User', id_group))
        elif action == "Top":
            if type_user == 'Admin':
                await state.set_state(Form.TuesdayTimetable)
...
async def SetTuesday(message: Message, state: FSMContext) -> None:
    try:
        ...
        if path and id_group:
            await state.update_data(TuesdayTimetable=path)

            if action == "Lower":
                cursor.execute("""UPDATE KNEU SET timetable_tuesday_lower = ? WHERE id = ?""", (path, id_group))
                conn.commit()
                ...
                cursor.execute("""UPDATE KNEU SET timetable_tuesday_top = ? WHERE id = ?""", (path, id_group))
                conn.commit()
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 27</b>. Задля її усунення зверніться будь ласка до @Zakhiel")
        ...
        if action == 'Lower':
            if type_user == 'Admin':
                ...
                if timetable == "Розкладу на середу нижнього тижня ще немає":
                    ...
                    await call.message.answer_photo(photo, "<b>Розклад на середу нижнього тижня</b>", show_caption_above_media=False   , reply_markup=BackKb('WeekSelection', 'User', id_group))
        elif action == "Top":
            if type_user == 'Admin':
                await state.set_state(Form.WednesdayTimetable)
...
async def SetWednesday(message: Message, state: FSMContext) -> None:
    try:
        ...
        if path and id_group:
            await state.update_data(WednesdayTimetable=path)

            if action == "Lower":
                cursor.execute("""UPDATE KNEU SET timetable_wednesday_lower = ? WHERE id = ?""", (path, id_group))
                conn.commit()
                ...
                cursor.execute("""UPDATE KNEU SET timetable_wednesday_top = ? WHERE id = ?""", (path, id_group))
                conn.commit()
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 29</b>. Задля її усунення зверніться будь ласка до @Zakhiel")
        ...
        if action == 'Lower':
            if type_user == 'Admin':
                ...
                if timetable == "Розкладу на четвер нижнього тижня ще немає":
                    ...
                    await call.message.answer_photo(photo, "<b>Розклад на четвер нижнього тижня</b>", show_caption_above_media=False   , reply_markup=BackKb('WeekSelection', 'User', id_group))
        elif action == "Top":
            if type_user == 'Admin':
                await state.set_state(Form.ThursdayTimetable)
...
async def SetThursday(message: Message, state: FSMContext) -> None:
    try:
        ...
        if path and id_group:
            await state.update_data(ThursdayTimetable=path)

            if action == "Lower":
                cursor.execute("""UPDATE KNEU SET timetable_thursday_lower = ? WHERE id = ?""", (path, id_group))
                conn.commit()
                ...
                cursor.execute("""UPDATE KNEU SET timetable_thursday_top = ? WHERE id = ?""", (path, id_group))
                conn.commit()
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 31</b>. Задля її усунення зверніться будь ласка до @Zakhiel")
        ...
        if action == 'Lower':
            if type_user == 'Admin':
                ...
                if timetable == "Розкладу на п'ятницю нижнього тижня ще немає":
                    ...
                    await call.message.answer_photo(photo, "<b>Розклад на п'ятницю нижнього тижня</b>", show_caption_above_media=False , reply_markup=BackKb('WeekSelection', 'User', id_group))
        elif action == "Top":
            if type_user == 'Admin':
                await state.set_state(Form.FridayTimetable)
...
async def SetFriday(message: Message, state: FSMContext) -> None:
    try:
        ...
        if path and id_group:
            await state.update_data(FridayTimetable=path)

            if action == "Lower":
                cursor.execute("""UPDATE KNEU SET timetable_friday_lower = ? WHERE id = ?""", (path, id_group))
                conn.commit()
                ...
                cursor.execute("""UPDATE KNEU SET timetable_friday_top = ? WHERE id = ?""", (path, id_group))
                conn.commit()
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 33</b>. Задля її усунення зверніться будь ласка до @Zakhiel")
        ...
        if action == 'Lower':
            if type_user == 'Admin':
                ...
                if timetable == "Розкладу на суботу нижнього тижня ще немає":
                    ...
                    await call.message.answer_photo(photo, "<b>Розклад на суботу нижнього тижня</b>", show_caption_above_media=False   , reply_markup=BackKb('WeekSelection', 'User', id_group))
        elif action == "Top":
            if type_user == 'Admin':
                await state.set_state(Form.SaturdayTimetable)
...
async def SetSaturday(message: Message, state: FSMContext) -> None:
    try:
        ...
        if path and id_group:
            await state.update_data(SaturdayTimetable=path)

            if action == "Lower":
                cursor.execute("""UPDATE KNEU SET timetable_saturday_lower = ? WHERE id = ?""", (path, id_group))
                conn.commit()
                await message.answer(f"Розклад на суботу нижнього тижня успішно збережено", reply_markup=DaysKeyboard('Lower', 'WeekSelection', 'Admin', id_group))
            elif action == "Top":
                cursor.execute("""UPDATE KNEU SET timetable_saturday_top = ? WHERE id = ?""", (path, id_group))
                conn.commit()
                await message.answer(f"Розклад на суботу верхнього тижня успішно збережено", reply_markup=DaysKeyboard('Top', 'WeekSelection', 'Admin', id_group))
            else:
                await message.answer("Виникла помилка")
        ...
        await message.answer(f"Виникла помилка: <code>{e}</code>. <b>ID: 35</b>. Задля її усунення зверніться будь ласка до @Zakhiel")
